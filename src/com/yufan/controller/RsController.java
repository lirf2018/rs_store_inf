...
import com.yufan.common.util.ResponeUtil;
import com.yufan.pojo.TbInfAccount;
import com.yufan.service.BasicService;
...
import com.yufan.common.util.VerifySign;
import com.yufan.service.ResultOut;
import com.yufan.service.ServiceFactory;
...
public class RsController {
    ...
    private static Logger log = Logger.getLogger(RsController.class);
...
//                log.info("系统参数为空校验结果=" + index + "---->注 1:req_type为空 2:sid为空 3:data为空 4:timestamp为空 5:sign为空0:正常 ");
                if (null == index || index != 0) {
                    ...
                    if (betweenTime < 120) {//秒
                        //验证sign(根据账号sid查询密钥)
                        TbInfAccount account = basicService.loadTbInfAccount(jsonHeaderBean.getSid());
                        String appsecret = account.getSecretKey();
//                        log.info("appsecret="+appsecret);
                        //检验签名
                        if (VerifySign.checkSign(jsonHeaderBean, appsecret)) {
                            ResultOut resultOut = ServiceFactory.getService(jsonHeaderBean.getReq_type());
    ...
    public void testLog(HttpServletRequest request, HttpServletResponse response) {
        String result = "";
        PrintWriter pw = null;
        String message = null;
        try {
            ...
            log.info("接收参数");
            log.info(message);
            result = packagMsg(ResultCode.OK.getResp_code(), null);
            pw.write(result);
            pw.flush();
            pw.close();
            return;
