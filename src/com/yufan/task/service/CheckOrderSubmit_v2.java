...
/**
 * 创建人: lirf
 * 创建时间:  2018-01-30 14:57
 * 功能介绍: 下单校验(本次改版支持全国地址)
 */
@Service("check_order_v2")
public class CheckOrderSubmit_v2 implements ResultOut {

    private Logger LOG = Logger.getLogger(CheckOrderSubmit_v2.class);
    ...
    public String getResult(ReceiveJsonBean receiveJsonBean) {
        ...
        try {
            ...
            return saveOrderData(orderBean);
        } catch (Exception e) {
     ...
     * @return
     */
    public synchronized String saveOrderData(OrderBean orderBean) {
        try {
            //验证订单商品
            ...
            if (null == orderDetailBeansList || orderDetailBeansList.size() == 0 || orderDetailBeansList.size() > 11) {
                ...
                return packagMsg(ResultCode.GOODSINFO_MAX_OUT.getResp_code(), new JSONObject());
            }
            ...
            Integer userAddrId = orderBean.getUserAddrId();//收货地址地址标识(根据当前系统配置确定收货方式)
            int orderGoodsCountParam = orderBean.getGoodsCount();//订单数量 = 商品数量总和
            BigDecimal postPriceParam = orderBean.getPostPrice();//

            int addrPeiType = -1;//配送方式  地址类型0.自取 1配送
            /**
             * 配送费
             */
            ...
            if (addrType == 2) {
                ...
                if (!StringUtils.nonEmptyString(orderBean.getUserPhone()) || !StringUtils.nonEmptyString(orderBean.getUserName())) {
                    ...
                    return packagMsg(ResultCode.ADDR_NOT_EXIST.getResp_code(), new JSONObject());
                }
                TbDistributionAddr distributionAddr = CacheData.platformMap.get(userAddrId);//orderBean.getUserAddrId()平台地址标识
                if (null == distributionAddr) {
                    LOG.info("----->平台配送或者自取地址不存在,标识:  " + orderBean.getUserAddrId());
                    return packagMsg(ResultCode.ADDR_NOT_EXIST.getResp_code(), new JSONObject());
                }
                addrPeiType = distributionAddr.getAddrType();//平台地址类型4.自取 5配送 6 还货
            ...
            if (postPriceParam.compareTo(postPrice) != 0) {
                ...
                return packagMsg(ResultCode.ORDWE_PRICE_ERROR.getResp_code(), new JSONObject());
            }
            ...
            for (int i = 0; i < orderDetailBeansList.size(); i++) {
                ...
                BigDecimal nowPrice = goods.getNowMoney();//---------实时现价(销售价格,根据商品可能会变)
                ...
                if (goodsGetWay != getWay) {
                    LOG.info("---->商品取货方式不一致");
                    return packagMsg(ResultCode.GOODS_GETWAY_DIFF.getResp_code(), new JSONObject());
                }

                //--------------------------------------------------价格计算
                //商品押金
                goodsDepositPriceStore = goodsDepositPriceStore.add(depositMoney.multiply(new BigDecimal(goodsCountParam)));
                if (isPayOnline == 0) {//是否线上支付 0可以不用线上支付
                ...
                if (goodsOrderSingle && orderDetailBeansList.size() > 1) {
                    ...
                    return packagMsg(ResultCode.ORDER_ALONE_ERROR.getResp_code(), new JSONObject());
                }
                //判断是否是商品券
                if (goodsOrderSingle && orderDetailBeansList.size() > 1) {
                    LOG.info("---------->商品券(单个生成订单)");
                ...
                if (0 != limitGoodsNum && limitGoodsWay != 4) {
                    ...
                    if (goodsCountParam > limitGoodsNum) {
                        ...
                        return packagMsg(ResultCode.ORDER_LIMIT_ERROR.getResp_code(), new JSONObject());
                    }
                    //商品限购规则(限购时间开始之后的数据)
                    if (1 == limitGoodsWay) {
                        //1.每天一次
                ...
                if (isSingleG == 1) {
                    ...
                    if (goodsCountParam > goodsStore) {
                        ...
                        return packagMsg(ResultCode.GOODS_STORE_NOENOUGH.getResp_code(), new JSONObject());
                    }
                    //判断是否是抢购商品
                    if (timeGoodsIdParam > 0) {
                        ...
                        if (StringUtils.nonEmptyString(CacheData.timeGoodsOut)) {
                            ...
                            if (null == passTime || "".equals(passTime) || DatetimeUtil.compareDateTime(nowTime, passTime) == 1) {
                                ...
                                return packagMsg(ResultCode.FAIL_GOODS_INVALIDATE.getResp_code(), new JSONObject());
                            }
                        }
                        ...
                        if (isPayOnline == 1) {//是否线上支付 1只能线上支付
            ...
            if (orderGoodsCountParam != goodsCount) {
                ...
                return packagMsg(ResultCode.ORDWE_COUNT_ERROR.getResp_code(), new JSONObject());
            }
